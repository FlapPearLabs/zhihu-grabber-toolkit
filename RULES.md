...
6. 实现与 Applicable Approved Specs 冲突 → 按 `AGENTS.md` 的 conflict 流程 STOP，不得擅自改 Spec 迁就实现；
...
对仍有对应 remote branch 的本地 branch ref，按以下顺序执行；`<lost-branch>` 必须替换为已确认丢失的具体分支名：

```bash
git fsck --full
git fetch origin
git show-ref --verify --quiet "refs/heads/<lost-branch>" && { echo "local ref still exists; STOP"; exit 1; }
git show-ref --verify "refs/remotes/origin/<lost-branch>"
git update-ref "refs/heads/<lost-branch>" "$(git rev-parse "refs/remotes/origin/<lost-branch>")" 0000000000000000000000000000000000000000
```

`git update-ref` 的零旧值只允许在本地 ref 确认不存在时创建它；不得覆盖一个仍存在的 ref。若 remote ref 已不存在，只有在 `git fsck --full` / reflog 能对应到**已核验的精确 SHA**且恢复关系有记录时，才可将同一命令中的新 SHA 替换为该 SHA；否则 STOP，不得从 dangling object 猜测分支归属。

恢复后必须重新核验，而不是把命令成功当作恢复成功：

```bash
git fsck --full
git rev-parse "refs/heads/<lost-branch>"
git ls-remote --refs origin "refs/heads/<lost-branch>"
git merge-base --is-ancestor origin/master "refs/heads/<lost-branch>"
```

对于从 `origin/master` 创建的普通 task branch，最后一条必须退出码为 0，且恢复的本地 SHA 必须等于已获取的 `refs/remotes/origin/<lost-branch>`；否则 STOP。恢复的是 master、已合并历史或非 task ref 时，必须先按其已记录角色选择正确的 ancestry 方向并核验 remote ref 身份；无法证明则 STOP，不得继续提交、合并、关闭 Issue 或更新 Tracker。
