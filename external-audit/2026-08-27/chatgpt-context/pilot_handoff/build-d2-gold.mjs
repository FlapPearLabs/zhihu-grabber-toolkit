...
export function buildD2Gold({ adjudication, oldGold, caseCfg, pool, caseId }) {
  ...
  // ---- aspects: apply case_schema_decisions (KEEP only) ------------------------
  const oldAspects = (oldGold.families.aspect_membership && oldGold.families.aspect_membership.aspects) || [];
  const keepAspectDec = (a) => {
    const dec = schemaDec.aspects[a.aspect_id];
    if (dec === undefined) return false; // not mentioned -> not kept
    if (dec === 'KEEP') return true;
    if (dec === 'DROP') return false;
    return false;
  };
  const aspects = oldAspects.filter(keepAspectDec).map((a) => {
    const members = [...(aspectSources.get(a.aspect_id) || [])].filter((s) => relevanceTrue.has(s));
    return { aspect_id: a.aspect_id, name: a.name, definition: a.definition || a.name, primary_sources: members, sources: members, unresolved_sources: [], disputed_sources: [] };
  });
  ...
  const historicalAuthority = {
    ...
    sources: [],
    unresolved_sources: [...relevanceTrue, ...relevanceFalse, ...relevanceUnresolved],
    disputed_sources: [],
    note: 'adjudicated UNRESOLVED for all real case labels (packet omits timestamp/age evidence for long-established status); excluded from numerator and denominator',
  };
...
function main() {
  ...
  for (const caseId of REAL_CASES) {
    ...
    // backup D1 gold (historical), write D2 gold, rebuild value_units
    ...
    fs.writeFileSync(path.join(CASES, caseId, 'value-units.json'), JSON.stringify(units, null, 2));
