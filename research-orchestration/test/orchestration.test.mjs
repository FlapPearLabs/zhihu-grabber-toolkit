...
test('P1: percentage as SUBJECT (not corpus subset) MUST stay FULL-COVERAGE (R4, no silent downgrade)', () => {
  for (const t of [
    ...
    '用采样数据训练模型有什么问题',
    '采样率20%的信号处理方法',
    // 采样 as ordinary SUBJECT (not a subset-view request) → FULL-COVERAGE
    '给我讲讲采样定理',
    '要研究采样定理',
    '来分析采样数据的统计特征',
    '给我解释采样率的意义',
    '做采样实验需要注意什么',
    // 采样/抽样/只采样 as ordinary SUBJECT (no answer/corpus subset frame) → FULL-COVERAGE
    '做抽样分析需要注意什么',
    '抽样分析方法的优缺点',
    '抽样分析在质量控制中的应用',
    '只采样数据训练可以吗',
    ...
    '采样样本的分布',
    '研究采样摘要的生成',
    // percent as metric-noun compound (回答率/评论数/样本量) is NOT a corpus subset
    '我要20%的回答率',
    '我要20%的评论数',
    '我要20%的样本量',
    // 只采样/抽样分析/采样版 as SUBJECT (no answer/corpus noun frame) → FULL-COVERAGE
    '只采样部分数据训练可以吗',
    '只采样部分客户做试点',
    '只采样少量样本会丢失信息吗',
    '只采样一些样本进行标注',
    ...
    '采样版本与全量版本的差异',
    '采样版画风格研究',
    // methodology / feasibility / how-to questions (same subject class as required-FULL twins)
    '对回答做抽样分析需要注意什么',
    '对回答做抽样分析的方法',
    '对评论做抽样分析的优缺点',
    '如何对回答做抽样分析',
    '怎么对回答做抽样分析',
    '怎样对回答做抽样分析',
    '如何选择合适的抽样方法对前20%的回答进行抽样分析',
    '如何科学地选择抽样方法对前20%的回答进行抽样分析',
    '如何选择合适的方法来对前20%的回答进行抽样分析',
    '如何只采样部分回答做分析',
    '怎么只采样部分回答',
    '如何评估一个问题，结合多方意见，最终对回答做抽样分析',
    '如何科学合理地选择抽样方法、确定样本量并评估误差风险，以确保分析结果具有代表性，最终对前20%的回答进行抽样分析',
    '如何选择一套经过严格实践检验、在多个领域都有成功应用记录且被广泛认可的抽样方法对前20%的回答进行抽样分析',
    // suffix-position how-to / methodology markers (镜像语序)
    '对回答做抽样分析怎么做',
    '对回答做抽样分析有什么意义',
    '对回答做抽样分析有什么好处',
    '对回答做抽样分析应该怎么做',
    ...
    '对前20%的回答做抽样分析可以吗',
    '对前20%的回答做抽样分析是否合理',
    // comma-clause feasibility variants (comma must not defeat the guard)
    '只看前20%的回答，可以吗',
    '前20%的回答，是否合理',
    '选20%的回答做分析，可以吗',
    '前20%的回答，应该怎么看',
    ...
    '对回答做抽样分析，好不好',
    '只采样部分回答，能不能做',
    // feasibility variants of ALL sampled frames (只看高赞/快速看看/采样视图/不需要全量 …)
    '只看高赞回答，可以吗',
    '只看高赞回答，行不行',
    '只看高赞回答，是否可行',
    '只看高赞回答，好不好',
    ...
    '只看前20%的回答，好吧',
    '前20%的回答，行吧',
    // how-to prefix over other sampled frames
    '如何看前20%的回答',
    '怎么看前20%的回答',
    '如何只看前20%的回答',
    '如何快速看看新能源汽车',
