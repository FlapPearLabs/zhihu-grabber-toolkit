// Metric evaluator — D2.1 CORRECTED question-provenance semantics.
// D2.1 changes (relative to D2, the invalid/corrected pilot evaluator):
//   1. value units carry scope (QUESTION | CROSS_QUESTION | CASE) and a
//      canonical question_ids[] set (see value-units.mjs); question membership
//      NEVER depends on gold array order.
//   2. per_question_coverage counts a unit for EACH question in question_ids;
//      a unit is covered FOR question q only when a selected supporting
//      source FROM q exists (no cross-question credit).
//   3. CASE-scoped units (expert_source_group / evidence_source_group) never
//      enter any per-question denominator or minority_question_recall.
// Approved contract boundaries (unchanged, not reinterpreted):
...
import { scorableUnitsByQuestion, unitsCoveredForQuestion } from './value-units.mjs';
...
export function computeMetrics({ caseCfg, gold, pool, selected, cost, extra }) {
  ...
  // NOTE: in the Phase B race, run-race.mjs overrides this metric with the REAL
  // DENSE backend after evaluation (see scripts/run-race.mjs) so redundant
  // comparisons within the race use the same real dense vectors as selection.
  {
  ...
  // ---- Normalized Question Diversity (P0-4: Q = # scorable reference Qs; D2.1: question_ids) -----
  {
    const scorableQ = new Set();
    for (const u of valueUnits) if (u.scorable) for (const qid of u.question_ids || []) scorableQ.add(qid);
  ...
  // ---- Per-question coverage (P0-2: value units; D2.1: per-question credit) ----
  {
    ...
    for (const q of referenceQs) {
      ...
      // D2.1: a unit is covered FOR q only via a selected source FROM q
      const covered = unitsCoveredForQuestion(unitsQ, selectedSet, q).length;
    ...
    out.per_question_coverage_preservation = {
      ...
      provenance_semantics: 'D2.1: unit credit per question in question_ids; covered(q) requires selected supporting source FROM q; CASE units excluded',
    };

    // D2.1 diagnostic: cross-question value units and how many of their
    // questions actually get coverage credit (order-invariant provenance).
    {
      const xqUnits = valueUnits.filter((u) => u.scorable && u.scope === 'CROSS_QUESTION');
      const perXq = xqUnits.map((u) => {
        const coveredQs = (u.question_ids || []).filter((qid) => unitsCoveredForQuestion([u], selectedSet, qid).length > 0);
        return {
          unit_id: u.unit_id,
          question_ids: u.question_ids,
          covered_question_count: coveredQs.length,
          question_count: u.question_ids.length,
          covered_questions: coveredQs,
        };
      });
      out.cross_question_unit_credit_diagnostic = { units: perXq, note: 'diagnostic; per-question coverage credits a CROSS_QUESTION unit only via its own question sources' };
    }
