...
日期：2026-09-19。Repository：`FlapPearLabs/zhihu-grabber-toolkit`。性质：已记录 owner 决策的限定修复计划；不是 Approved Spec、实现授权或修复验收。本文 CD 的选择已冻结，所需正式 contract amendment 与独立审查尚未完成；“目标／必须”不表示当前产品已实现。决策权威与影响详见 [OWNER_CONTRACT_DECISIONS.md](OWNER_CONTRACT_DECISIONS.md)。
...
F-RP-01 = CLOSED (external PASS at 8919438; owner-provided review result)
...
**Owner 已冻结决策**（本次 owner 提示词明确批准；不是 Spec amendment 完成记录）：

| ID | 状态 | 已选决策摘要 | 受影响范围 |
|---|---|---|---|
| CD-B1 | OWNER_APPROVED | S1 anchor proposition family + 显式 ASSERTS/OPPOSES/UNRESOLVED；不采用 S2，不从 claim kind 推导跨组立场 | B 语义合同 |
| CD-B2 | OWNER_APPROVED | 独立 UNRESOLVED record，relationStatus=UNRESOLVED、supportBreadth=null，保留原 statement/完整 lineage；不因关系未知自动 fail whole run，不伪装旧类别。all-unresolved 最终 COMPLETE 仍受 §17 semantic acceptance 约束 | B 输出与语义验收 |
| CD-B3 | OWNER_APPROVED | semantic contract version bump + Seam D major/formal update；具体版本号由后续 amendment 确定。旧产物 HISTORICAL_ONLY，不猜 relation/breadth；完整原始 lineage 可 re-synthesis/re-evaluation | A/B、T15、frozen validator |
| CD-B4 | OWNER_APPROVED | relation/conflict、support breadth、group-local salience 正交；不新增 synthesis-level groupSalience；legacy category=LEGACY_DERIVED_VIEW，不反向驱动语义 | B 输出、diagnostics、兼容视图 |
| CD-A1 | OWNER_APPROVED | restart=new execution occurrence，不复用旧 derived research stages；新题不沿用旧 Plan/Coverage/Selection/Manifest/Claims/Synthesis；保留 canonical bytes，不改变 grab resume-merge | A restart |
| CD-A2 | OWNER_APPROVED | 闭包失效则 REUSED_COMPLETE=FALSE，拒绝当前复用并报告最早失效边界；等待显式 restart/另行授权恢复，不自动联网或昂贵重跑 | A stale COMPLETE |
| CD-A3 | OWNER_APPROVED | legacy 缺必要版本/provenance 则拒绝当前 reuse，保留历史 bytes；不猜测或静默迁移，未来迁移须可证明 provenance | A legacy checkpoint |
| CD-C1 | OWNER_APPROVED | metadata-only source 仍经允许的 T13 semantic analysis，明确 no extractable claim 后才计 analyzed；不跳过分析、不凭 metadata 发明 claim；全体无 claim 保持 fail closed | C/T13 accounting |
| CD-C2 | OWNER_APPROVED | Agent projection DEFAULT OMIT raw code body，仅允许安全 metadata；不静默发送完整外图 URL/file URI。代码正文语义能力另走 Product/Spec/Security decision | C 安全边界 |
| CD-D1 | OWNER_APPROVED | one successful resolution；非法输入不消耗成功机会，pending 冻结、不重问/重检索；相同成功答案可幂等重试，第二个不同成功选择拒绝，进程重启不重置 | D continuation |
| CD-F1 | NON_BLOCKING_FOR_CORE_REPAIR | D2_CURRENT_IMPLEMENTATION=VERIFIED、D2_ORIGINAL_APPROVAL_RECEIPT=NOT_LOCATED；Lane F 独立事实校准，不阻塞 A–E，不用 merge/tests 冒充历史批准 | F 文档，核心非阻塞 |

Owner 不需要再次选择这些项目。下一步独立 Contract + Consistency review 核对记录准确性、authority 冲突和 amendment 范围；未完成正式合同更新与所需审查，不得把 owner freeze 称为 execution-ready。本轮不拆票、不实施、不分派 agents。

## 2. CURRENT VERIFIED STATE

本节保留原计划起草时态；前次 F-RP-01 修订记录见 supporting/f-rp-01-self-review.md。本次 owner freeze 已 fresh fetch，master=4bea7b30b3842a876e383977686a0abc0d68302f，reviewed plan head=8919438c0103feeebd723355cfd262f7dcee9dc3，均匹配。外部 F-RP-01 PASS/CLOSED/0 blockers 按本次 owner 提供的结果记录，来源见 OWNER_CONTRACT_DECISIONS.md；历史 probe/完整测试未在本轮重跑。
...
|---|---|---|---|
| F01 | CONFIRMED / STILL_REPRODUCIBLE：新题发旧查询 | SAME_ROOT_CAUSE A；OWNER_DECISION_FROZEN / CONTRACT_REVIEW_REQUIRED | 高：错题研究 |
| F02 | CONFIRMED / STILL_REPRODUCIBLE：缺 plan/source 仍 COMPLETE | SAME_ROOT_CAUSE A；OWNER_DECISION_FROZEN / CONTRACT_REVIEW_REQUIRED | 高：依赖失效仍宣称可复用 |
| F03 | CONFIRMED / STILL_REPRODUCIBLE：相反主张共同 support；无关观点伪 oppose | OWNER_DECISION_FROZEN / CONTRACT_REVIEW_REQUIRED B | 最高语义风险：伪共识/伪冲突 |
| F04 | CONFIRMED / STILL_REPRODUCIBLE：真实 adapter 请求含 raw HTML/code/URL | 独立 C；继承合同恢复，CD-C1/C2 已由 owner 冻结，待 contract review | 高安全风险；未证执行代码/窃取凭据，不升级 P0 |
...
**CONTROLLER AUTHORITY**：模型响应缺字段/重复/未知 ID/空簇 → 现有 fail-closed 类，不自动修正或 fallback 到旧 aspect union。结构合法但关系不明 → 按 CD-B2；按已冻结 CD-B2 的独立 unresolved 列表保留有原始 statement 与 lineage 的 unresolved record（singleton展示，不是family；无anchor自关系和support/oppose关系），仍保留 analysis accounting，单独披露 semantic relation 未确定。此 reporting policy 已获 owner 批准，仍待正式 contract amendment；不允许 semantic downgrade，也不放宽 §17 正向命中门。

### 5.3 输出与正交语义约束（CD-B4 OWNER_APPROVED；待正式 amendment）
...
已解析 family 必有 anchor 自 ASSERTS，故支持组数不可能为 0；违反则拒绝，不补成 SINGLE_GROUP。独立 UNRESOLVED record 无 anchor 自关系、无 support/oppose，保留原出处及 `relationStatus=UNRESOLVED, supportBreadth=null`；它不能因兼容展示变成 group-specific、minority 或 SUPPORT_ONLY。整个 run 可保留这些未知记录而不因未知自动失败，最终 COMPLETE 仍须遵循 CD-B2 与 §17 semantic acceptance；已解析 family 的状态不能覆盖 run 中另外的 unresolved records。以上“已验证”包括 controller 的结构接受与 scope 可比的语义提案要求，不把 deterministic check 当作语义真值证明。
...
- P1 Spec §8.1 的组内 metadata 与 §8.3 的报告区分继续可见，但新版 canonical shape、legacy 降权及诊断读取规则必须先获得所需契约批准。当前 Spec/frozen seam 不在本轮修改范围，owner 已批准决策不冒充现行产品行为或已完成的 Spec/seam amendment。
...
CD-B1 已选择 S1；下表保留原设计比较记录，S2 为未采用的历史备选，不是当前待决选项。

| 维度 | S1：anchor family + relative stance（OWNER_APPROVED） | S2：完整 pairwise relation proposal |
|---|---|---|
...
历史备选 S2（本轮未采用）的原约束：必须在 bounded input 上提出全部无序 pairs，UNKNOWN显式保留；不得把未提及边默认成 SAME。只有明示且通过结构一致性审查的 EQUIVALENT 类才可合并；不相容/未知关系禁止 controller 通过多数票补全。无需引入 KG/NLI 服务，但复杂度对目前八个 finding 不合算。

已选 S1 是最小 current defect repair；不同时加入两套算法/自动 fallback。若后续独立反例证明 S1 无法表达必需复合命题，再另行裁决，而不是预建全 claim graph。若改变既有字段语义或 required observable shape，遵守 P1_SEAM_CONTRACTS_V1:45–53 的 major/version/re-review 规则；加字段并不自动证明兼容。CD-B3/B4 的 owner 决策已冻结；先完成正式 contract/version amendment 与所需审查，再更新 frozen fixtures，不许 fixture 倒逼语义。从 single category 改为正交 canonical dimensions 必须 semantic contract version bump 与 SEAM D major bump，不能把 legacy 字段仍存在当成 V1 语义兼容。旧产物仅 historical artifact，不猜补 relation/breadth；若原始 sourceClaim lineage 可取回并重新验证，可按新合同重新判断关系、重新综合（re-synthesis / re-evaluation）并生成新版本 identity，不能称 migration by assumption。raw lineage 缺失则不可转换。
...
| 有效plan bytes相同，但显式restart | MUST_REVALIDATE | restart政策优先；按 CD-A1 新执行，不以hash巧合取消用户意图 |
...
| legacy缺配置/prompt/projection/semantic版本或必要provenance | MUST_REFUSE_CURRENT_REUSE | CD-A3 已批准：保留历史 bytes，不猜旧版本、不静默迁移 |
...
stale COMPLETE 按已冻结 CD-A2 明确拒绝 current reuse（REUSED_COMPLETE=FALSE）并报告最早失效边界，保留历史；不自动联网或昂贵重跑，等待显式 restart/未来明确授权恢复；ordinary interrupted resume 应按最早无效阶段恢复并保留valid siblings，不能一律全量重抓。检查本身不得偷偷网络IO或改写canonical。对单次读取使用固定快照/已有TOCTOU防护，不宣称修复整个文件系统并发/断电事务性。
...
T08继续验证 canonical/unique IDs、eligible membership、全部required groupKeys与精确剩余slots；无material ambiguity不接受force-selection。一次成功resolution（CD-D1 OWNER_APPROVED；待合同澄清）；非法输入固定错误、零capture/model/provider、pending不变，不再问第二个问题；第二个不同成功选择拒绝。同一答案重试可复用已验证resolved decision，resume从已完成阶段继续，不重复capture。不能把每次CLI进程重启当clarification次数重置。
...
以下字段是未来执行边界，文件名表示可能受影响的范围，不是允许任意修改整个文件。精确allowlist须在独立 Contract + Consistency review、所需 amendment 及下一阶段授权之后拆票时固定；本轮不拆票或分派任务。
...
- **DEPENDENCIES / PARALLELISM**：CD-F1=NON_BLOCKING_FOR_CORE_REPAIR；后续窄doc review 独立推进，不以 receipt 缺席阻塞 Lane A–E 或其集成。
...
- **ACCEPTANCE**：diff仅对应文案/注释、无runtime语句变化、bounded search区分current/historical；按 CD-F1 保留 receipt 未找到的披露，不捏造历史批准。
...
```mermaid
flowchart TD
  CD[Owner decisions frozen; required review + amendment pending] --> AC[A reuse contract]
  ...
  FR[CD-F1 non-blocking; historical receipt not located] --> F[F docs + contract review; independent]
...
每条lane都有RED→implementation→GREEN，图中E0不是测试作者。E1逐lane路由不需等I；B/C的contract版本定义先给A，实际新版本与产物在I重新验证。A中F01/F02共享判断，不拆两个并发writer；B0可以先做纯非空约束，但不能先发明 stance/正交状态/legacy 视图定义。F独立，不成为 A–E 实现或核心集成的前置；图中移除 F→I 仅同步 CD-F1 非阻塞决定。集成master按现有serial exact-SHA规则，图不构成merge授权。

## 15. EXECUTION WAVES

| Wave | 内容 | 并行及停止条件 |
|---|---|---|
| 0 | owner CD 表已冻结；独立 Contract+Consistency review 与必要正式 amendment（具体版本号在该阶段确定）；E0测试清单；F 独立事实校准，保留 receipt 未找到披露 | 不实施产品。待正式更新/审查的 contract 对应lane停；F 不阻塞 A–E；普通固定测试策略无需重开产品方向 |
...
将本候选的远端 branch / exact SHA 交给独立 CONTRACT_REVIEWER + CONSISTENCY_REVIEWER：只检查 [owner decision record](OWNER_CONTRACT_DECISIONS.md) 是否准确记录 owner 选择、与 Repair Plan 是否一致、哪些正式 authority amendment 必需，以及现行 authority 的冲突/澄清点。F-RP-01 保持 CLOSED；本次不重开设计、不将前次 PASS 转移成当前双审 PASS。

Owner decisions 已 FROZEN；P1 Spec / Seam D 等必要正式 amendment 仍未完成。具体 authority impact 与 CONTRACT_REVIEW_REQUIRED 事项见 decision record；Spec/seam 文件本轮保持原样，不自行消解冲突。通过后续所需审查与 amendment 也不自动产生本轮没有授予的实现/拆票权限。

本轮仅从 8919438c0103feeebd723355cfd262f7dcee9dc3 创建新的 owner-decision 分支，提交/推送授权的计划包文件；不覆盖原分支、不创建 PR、不 merge、不改 product/tests/Spec/seam/AGENTS/RULES/project-memory/Issues/CI，不执行 F01–F08 修复，不分派 agents、不写实施 prompts。

本次窄一致性自检见 [supporting/owner-decision-self-check.md](supporting/owner-decision-self-check.md)。历史 supporting receipts 与 evidence 保持原文，旧 PENDING 状态仅描述旧 SHA；当前 owner 状态以 OWNER_CONTRACT_DECISIONS.md 为准。本轮未运行产品测试或 live semantic validation。
