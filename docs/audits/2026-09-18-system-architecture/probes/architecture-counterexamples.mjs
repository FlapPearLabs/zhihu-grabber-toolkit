// Publication copy: imports are repository-relative; outputs are OS-temporary.
// Fixture helpers below were copied from the audited wiring test; reviewer-owned probes start at the composer import.
...
import { planHash } from '../../../../research-orchestration/lib/plan-contract.mjs';
import { createProviderSeam, CAPABILITY_SEARCH, AUTH_CLASS_OFFICIAL_SECRET } from '../../../../research-orchestration/lib/provider-seam.mjs';
import { P1_PIPELINE_IDENTITY } from '../../../../research-orchestration/lib/coverage-final-integration.mjs';
import { makeState, readState, STAGE_SEARCH } from '../../../../research-orchestration/lib/state.mjs';
import { REQUIRED_EMBEDDING_IDENTITY } from '../../../../research-orchestration/lib/dense-geometry.mjs';
import { mockVector768 } from '../../../../research-orchestration/test/helpers/test-embedding-provider.mjs';
import { T14_SYNTHESIS_RUNTIME_ID, T14_SYNTHESIS_MODEL } from '../../../../research-orchestration/lib/cross-source-synthesis.mjs';
...
import { composeP1Research } from '../../../../research-orchestration/lib/p1-runtime-composer.mjs';

const EVIDENCE = fs.mkdtempSync(path.join(os.tmpdir(), 'zhihu-audit-repro-'));
const report=[];
const workDir=fs.mkdtempSync(path.join(EVIDENCE,'architecture-checkpoint-'));
const baseOpts={topic:'AI 编程工具会取代程序员吗', workDir, runtime:pinnedMockRuntime(), seam:rankingSeam(['100','200']), captureAdapter:captureAdapterFor({100:answersJsonFor('100',GROUP_100_TEXTS),200:answersJsonFor('200',GROUP_200_TEXTS)}),runner:groupRunnerFor(),embeddingProvider:testEmbeddingProvider()};
const first=await composeP1Research({...baseOpts,plan:PLAN});
assert.equal(first.ok,true,JSON.stringify(first));
const planPath=path.join(workDir,'research-plan.json');
const originalPlan=fs.readFileSync(planPath);
fs.unlinkSync(planPath);
const missingPlan=await composeP1Research(baseOpts);
report.push({case:'complete_missing_research_plan',expected:'refuse stale completed checkpoint',observed:{ok:missingPlan.ok,reused:missingPlan.reused,complete:missingPlan.result?.disclosure?.complete},confirmed:missingPlan.ok&&missingPlan.reused});
fs.writeFileSync(planPath,originalPlan);
function findAnswers(dir){return fs.readdirSync(dir,{withFileTypes:true}).flatMap(e=>e.isDirectory()?findAnswers(path.join(dir,e.name)):e.name==='answers.json'?[path.join(dir,e.name)]:[])}
const sourceFiles=findAnswers(workDir);
assert.ok(sourceFiles.length>0);
const source=sourceFiles[0]; const sourceOriginal=fs.readFileSync(source); fs.unlinkSync(source);
const missingSource=await composeP1Research(baseOpts);
report.push({case:'complete_missing_canonical_answers',removed:path.relative(workDir,source),expected:'refuse stale dependency',observed:{ok:missingSource.ok,reused:missingSource.reused,complete:missingSource.result?.disclosure?.complete,isFullCoverage:missingSource.result?.disclosure?.isFullCoverage},confirmed:missingSource.ok&&missingSource.reused});
fs.writeFileSync(source,sourceOriginal);
const calls=[];
const spySeam=createProviderSeam({adapters:[fixtureSearchAdapter('fixture-official',({query})=>{calls.push(query);return searchResult('fixture-official',[],{ok:false,failure:{code:'PROVIDER_TRANSPORT_FAILED',class:'transport'}})})]});
let fetchCalls=0;
const restartResult=await composeP1Research({...baseOpts,topic:'儿童近视防控',restart:true,seam:spySeam,fetchImpl:async()=>{fetchCalls++;throw new Error('offline probe must not perform network')}});
report.push({case:'restart_different_topic',expected:'plan for new topic or fail closed before old retrieval',observed:{code:restartResult.code,fetchCalls,queries:calls,newStateTopic:readState(workDir).topic,planUnchanged:Buffer.compare(fs.readFileSync(planPath),originalPlan)===0},confirmed:calls.includes(PLAN.queryVariants[0])});
const ambiguousDir=fs.mkdtempSync(path.join(EVIDENCE,'architecture-clarification-'));
const ambiguousPlan={...PLAN,sourceGroupIntents:[{intent:'代表性讨论',constraints:[],groupKey:null}]};
const ambiguityOpts={...baseOpts,workDir:ambiguousDir,plan:ambiguousPlan,seam:rankingSeam(['100','200','300'])};
const ambiguityFirst=await composeP1Research(ambiguityOpts);
const ambiguitySecond=await composeP1Research({...ambiguityOpts,clarification:{forceGroupIds:['100']}});
report.push({case:'clarification_resolution_ignored',expected:'valid clarification can continue same run',observed:{firstCode:ambiguityFirst.code,secondCode:ambiguitySecond.code,firstOptions:ambiguityFirst.options,secondOptions:ambiguitySecond.options,finalStage:readState(ambiguousDir).stage},confirmed:ambiguityFirst.code==='clarification_required'&&ambiguitySecond.code==='clarification_required'});
console.log(JSON.stringify(report,null,2));
fs.writeFileSync(path.join(EVIDENCE,'architecture-counterexamples.json'),JSON.stringify(report,null,2)+'\n');
