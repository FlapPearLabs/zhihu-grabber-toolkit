...
- **T17 adapter durable facts（T17 实现合并后沉淀；reviewer 裁定 POST_GATE_MEMORY_UPDATE_REQUIRED = YES）**：
  ...
  - **重复裁决权归融合层**：per-response 同题重复项必须原样透传，adapter 不得自行 per-response 去重（否则会引入 item-order-dependent 贡献，绕过融合层既有的重复裁决）；同通道内重复的最终裁决归属融合层，按现行 D2 合同执行——同 question 取最低 explicit rank 一次贡献、equal-best 等价折叠、equal-best 冲突 fail-closed 为 `FUSION_DUPLICATE_CONFLICT`、非法 rank 仍 fail、跨通道各自独立贡献。注：原始 `FUSION_DUPLICATE_IN_CHANNEL` 导出已冻结保留，但当前生产不再将其作为全通道 fatal 门；现行 D2 行为由既有测试（rrf.test.mjs D2-CE1…CE8）核验，不追认历史 owner approval——其原始批准 receipt 尚未找到（D2_ORIGINAL_APPROVAL_RECEIPT = NOT_LOCATED）。
...
- **v0.3 render 消费缝（RUN-BOUND 合同）**：`orchestrator.mjs loadP1FinalCoverage(workDir, expectedPlanHash)` —— 工作目录中裸 `coverage-final.json` 的存在**永远不足以**被消费：只有 run 的 orchestration state 显式声明 `p1FinalCoveragePlanHash`（由 P1 集成 composer 写入）且产物 planHash 与绑定精确相等时才消费，100% 声明只来自其对账断言（partial 不渲染为 complete）。未绑定的 v0.3 run 忽略任何外来/陈旧/畸形产物（行为不变，防跨 run 污染把 sampled run 渲染成 complete）；绑定命中但产物缺失/畸形/planHash 不匹配 → `coverage_failed` fail closed。
- **multi-group 复用权威委派（durable）**：integration owner 不得自写 planHash-only 复用检查——multi-group state 的复用/失效只能经冻结 T09 权威 `resumeMultiGroupExecution`（绑定 planHash + selectionIdentity + selectionDecisionHash 并逐产物 revalidate）。同 planHash 下 selection 漂移（如 clarification 扩选）必须 fresh-create，否则会静默欠执行新增组并伪造 100% 断言。已知平台限制（pre-existing，T09 层）：Windows 反斜杠 refs 使 T09 load 形状门拒绝真实持久化状态 → 降级为 fail-closed fresh-create（安全方向，无陈旧复用）。
